			...
			<div class="col-12">
				<table class="responsive-table">
					<thead>
						<tr>
							...
							<th scope="col-2">해지승인</th>

						</tr>
					</thead>
					<tbody>
						<c:forEach items="${cancellationList}" var="item">
							<tr>
								<td>${item.policyNumber}</td>
								<td>${item.name}</td>
								<td>${item.contractStatus}</td>
								<td><button
										onclick="location.href='/contract/contractCancellation/${item.policyNumber}' "
										class="btn btn-primary">해지승인</button></td>
							</tr>
						</c:forEach>
				...
				<div class="row mt-3 text-center">
					...
					<c:forEach begin="${startPage }" end="${endPage }" var="page">
						<a href="/admin/cancellation/${page }">${page }</a>
					</c:forEach>
					<a href="/admin/cancellation/${maxPage}">&raquo;</a>
				</div>
